...
btnaddRoom.addEventListener('click', function () {
    ...
    for (i = 1; i <= nbrRoom; i++) {
        ...
        hoteldiv.innerHTML += `
            ...
            <select class="form-control " name="pension[${i}][idpension]">
                    <option value="" id="pension${i}" disabled="" selected="">Select type of Pension</option>
                    <option value="13">Complete</option>
                    <option value="14">Breakfast / Lunch</option>
                    <option value="15">Breakfast / Dinner</option>
                    <option value="16">without</option>
                </select>
            </div>
    `
    }
...
document.addEventListener("change", function (e) {
    ...
    for (let i = 1; i <= nbchild; i++) {
        ...
        <option value="8">no supplement child bed 0 DH</option> 
        <option value="9">child bed supplement 25% single room</option>`

        } else if (e.target.id == `child-age-${i}` && e.target.value == "2.14") {
            document.querySelector(`#select-offer${i}`).innerHTML = ``
            document.querySelector(`#select-offer${i}`).innerHTML += `
        <option value="10">add 50% single room</option> `
        } else if (e.target.id == `child-age-${i}` && e.target.value == "14") {
            document.querySelector(`#select-offer${i}`).innerHTML = ``
            document.querySelector(`#select-offer${i}`).innerHTML += `
        <option value="11">add a single room</option>
        <option value="12">add 70% single room + bed</option>
        `
        }
